...
int ListaAdy::InsertarAdy(char ori, char des, int peso) {
  ...
  if (existeOri == -1 ||
      existeDes == -1) // No existe Orig o Dest. NO se inserta
    return -1;
  ...
  if (existeAdy == 0) // YA existe la adyacencia. NO se nserta
    return -2;

  posicAdy->sigAdy =
      adya; // NO existe la adyac. Se inserta al final de la lista
  adya->dirNod = posicDes;
