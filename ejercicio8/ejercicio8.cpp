...
int Estructura::insertar(std::string nuevo) {
  ...
  // Paso 2: Aumentamos el valor de final y en caso de que principio sea -1 lo
  // aumentamos 1 también
  final++;
  if (principio == -1) {
    principio++;
  }
...
std::string Estructura::extraer() {
  ...
  aux = cola[principio];
  // Paso 3: Pasamos nuestro valor de la posición extraída por nuestro valor por
  // default
  cola[principio] = "- -";
...
void menu() {
  ...
  do {
    ...
    case 1:
      ...
      if (temp == 0)
        std::cout << "Se añadío el valor con éxito" << std::endl;
      break;
    case 2:
      ...
      if (ret != "vacio") {
        std::cout << "Se extrajo el valor con éxito" << std::endl;
